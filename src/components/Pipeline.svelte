        ...
        <div class="token-stack">
          {#each result.tokens as token, i}
            <div class:current={i===history.length-1} class="token-row">
              ...
              <VectorStrip data={token} kind="state" active={selectedCol===i} height={34} width={24} />
            </div>
          {/each}
        </div>
        ...
        <div class="qkv-stack">
          {#each result.q as q, i}
            <div class:current={i===history.length-1} class="qkv-row">
              <VectorStrip data={q} kind="q" active={selectedRow===i} height={34} width={24}/>
              <VectorStrip data={result.k[i]} kind="k" active={selectedCol===i} height={34} width={24}/>
              <VectorStrip data={result.v[i]} kind="v" active={selectedCol===i} height={34} width={24}/>
            </div>
          {/each}
        </div>
...
.qkv-row{display:grid;grid-template-columns:repeat(3,24px);gap:5px;padding:2px 5px;border-radius:7px}.qkv-legend{display:grid;grid-template-columns:repeat(3,24px);gap:5px;text-align:center;font-size:11px}.q{color:#4f74ff}.k{color:#df5f5f}.v{color:#3d9d70}
.matrices{display:flex;gap:12px;align-items:flex-start}.matrices>div{flex:0 0 195px;background:#fbfbfd;border:1px solid #eceef2;border-radius:12px;padding:8px}.matrices b{display:block;text-align:center;font-size:11px;color:#697386;margin-bottom:3px}
