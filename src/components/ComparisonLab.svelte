<script>
  ...
  function toggle() {
    ...
    if (currentIndex < trace.length - 1) currentIndex = trace.length - 1;
    running = !running;
  }
  ...
  onMount(() => {
    ...
    const frame = now => {
      ...
      if (running && run && !run.done) {
        accumulator += delta;
        let guard = 0;
        while (accumulator >= PHYSICS.tau && !run.done && guard < 4) {
          stepComparison(run);
      ...
      if (run?.done && currentIndex >= run.trace.length - 1) running = false;
...
.replay-controls{display:grid;grid-template-columns:auto auto auto minmax(160px,1fr) auto;gap:7px;align-items:center;margin-top:11px}.replay-controls button{border:1px solid #d9dde5;border-radius:7px;background:#fff;padding:6px 9px;font-size:9px;cursor:pointer}.replay-controls .primary{background:#293548;color:#fff;border-color:#293548}.replay-controls button:disabled{opacity:.35}.replay-controls span{font:8px ui-monospace,SFMono-Regular,Menlo,monospace;color:#78818f}.replay-controls input{width:100%}
